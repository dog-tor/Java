...
if (gotName)
{
	...
	if (selection == 0)
	{
		output1 = output1 + pizzaSize[0] + " ";
		gotLarge = true;
	}
	else if (selection == 1)
	{
		output1 = output1 + pizzaSize[1] + " ";
		gotMedium = true;
	}
	else if (selection == 2)  // small pizza
	{
...
else
{
	...
	case 1: output1 = output1 +pizzaStyle[1] +
			...
			// deep dish
	case 2: output1 = output1 +pizzaStyle[2] +
			" pizza with\n";
    		gotDeepDish = true;
    		break;
...
double paid, change;
	...
	// grab what the customer pays us
	// and parse it to the data type double 
...

{
	...
	// see which toppings were selected
...
if (source == rbtCash || source == rbtCard)
{
	...
	// additional charge of 50 cents added
...
{
	// determine which size pizza was selected
			// then add appropriate cost
		if (gotLarge)
		{
			totalCost +=LG_COST;
		}
...
{
		// determine which style pizza was selected
		// then add appropriate cost
	if (gotHandTossed)
	{
		totalCost += HT_COST;
